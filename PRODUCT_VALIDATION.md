...
| Package structure | One package with subpath exports | `nexus-launch` with core, chain, Flap, Pons, Node metadata, and MCP subpaths |
| Terminal interface | Thin `nexus` executable over the library, with stable JSON output | `nexus`, `nexus-launch`, and `nexus-mcp` binaries |
...
| npm name/scope | Verify availability before scaffolding | `nexus-launch` available; `nexus-cli` and `cli-nexus` taken, and `clinexus` rejected as too similar to `cli-nexus` |
